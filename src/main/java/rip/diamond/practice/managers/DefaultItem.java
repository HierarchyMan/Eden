package rip.diamond.practice.managers;

import lombok.Getter;
import org.bukkit.Material;
import org.bukkit.inventory.ItemStack;
import rip.diamond.practice.util.ItemBuilder;
import rip.diamond.practice.util.serialization.BukkitSerialization;

import java.util.Arrays;
import java.util.List;
import java.util.function.Supplier;
import java.util.stream.Collectors;

@Getter
public enum DefaultItem {

    GOLDEN_HEAD("GOLDEN_HEAD", 
            null,
            ""),
    
    INSTA_BOOM_TNT("INSTA_BOOM_TNT",
            () -> new ItemBuilder(Material.TNT)
                    .name("&c&lInsta Boom TNT")
                    .lore(
                            "&7Explodes instantly when placed!",
                            "",
                            "&ePlace to thwack away the opponent"
                    )
                    .glow()
                    .build(),
            null);

    private final String name;
    private final Supplier<ItemStack> itemSupplier;
    private final String base64;

    DefaultItem(String name, Supplier<ItemStack> itemSupplier, String base64) {
        this.name = name;
        this.itemSupplier = itemSupplier;
        this.base64 = base64;
    }

    public ItemStack toItemStack() {
        // Prefer ItemBuilder/Supplier pattern over Base64
        if (itemSupplier != null) {
            return itemSupplier.get();
        }
        
        // Fallback to Base64 for legacy items
        if (base64 != null && !base64.isEmpty()) {
            try {
                return BukkitSerialization.itemStackFromBase64(base64);
            } catch (Exception e) {
                // Log error but don't crash - Base64 might be corrupted or incompatible
                System.err.println("[Eden] Failed to deserialize Base64 for item '" + name + "': " + e.getMessage());
                System.err.println("[Eden] This item will not be available. Consider regenerating with /eden item " + name);
                return null;
            }
        }
        
        return null;
    }

    public static DefaultItem getByName(String name) {
        for (DefaultItem item : values()) {
            if (item.getName().equalsIgnoreCase(name)) {
                return item;
            }
        }
        return null;
    }

    /**
     * Get a list of all item type names for tab completion.
     * @return List of all default item names
     */
    public static List<String> getNames() {
        return Arrays.stream(values())
                .map(DefaultItem::getName)
                .collect(Collectors.toList());
    }
}
